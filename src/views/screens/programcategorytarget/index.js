...
import ProgramCategoryTarget from "./ProgramCategoryTarget";

const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "programcategorytarget";
  ...
  const [msgObj, setMsgObj] = React.useState({ isOpen: false });
        ...
        <div class="mainContainer ">
          ...
          <Switch>
            <Route
              path={`${path}/`}
              render={(props) => (
                <ProgramCategoryTarget {...props} openNoticeModal={openNoticeModal} />
              )}
            ></Route>
          </Switch>
          <Notification closeNoticeModal={closeNoticeModal} msgObj={msgObj} {...props} />
        </div>
      </div>
    )
  );
