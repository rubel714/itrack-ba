...
import RoleToMenuPermission from "./RoleToMenuPermission";
// import "../../../assets/css/pgdatacollection.css";

// receive.css
const Index = (props) => {
  const { path } = useRouteMatch();
  const menukey = "roletomenupermission"; // this is in t_menu table
      ...
      <div class="pb-30">
        ...
        <Switch>
          <Route
            path={`${path}/`}
            render={(props) => (
              <RoleToMenuPermission {...props} openNoticeModal={openNoticeModal}  permissionType={permissionType}/>
            )}
          ></Route>
        </Switch>
        <Notification
